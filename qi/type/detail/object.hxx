...
namespace detail
{
  ...
  ManagedObjectPtr managedObjectFromSharedPtr(
      ...
      const boost::optional<ObjectUid>& maybeObjectUid = boost::none)
  {
    return ManagedObjectPtr(
            new GenericObject(oit, other.get(), maybeObjectUid),
      ...
      std::true_type)
  {
    ...
    const auto realImplObjectUid = os::ptrUid(other.get());
    auto localProxy = boost::make_shared<SyncProxyType>(other);
    return managedObjectFromSharedPtr(qi::Object<To>::interface(), std::move(localProxy), realImplObjectUid);
  }
}
